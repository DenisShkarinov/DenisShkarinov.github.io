...
countdown();function countdown(){let clock=document.querySelectorAll('#clock div');let date=new Date(2021,10,25,12,30,30);let dateNow=new Date();let dateMs=Date.parse('2021-10-25T12:30:30.000-07:00'),dateNowMs=Date.parse(dateNow);let dateDiff=Math.round((dateMs-dateNowMs)/(1000*60*60*24));let days=dateDiff,hours=date.getHours(),minutes=date.getMinutes(),seconds=date.getSeconds();seconds-=1;if(seconds==0){minutes--;if(minutes==0){hours--;if(hours==0){days--;}}}
